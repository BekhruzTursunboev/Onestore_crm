...
export default function ClientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleAddTransaction(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      form.reset();
      setModalOpen(false);
      fetchClient(); // Reload client details
    } catch (err) {
  ...
  if (loading) return (
    <div className="p-24 flex justify-center"><div className="w-6 h-6 border-2 border-white/10 border-t-white rounded-full animate-spin" /></div>
  );

  if (error || !client) return (
    <div className="p-12 text-center text-white/50">Mijoz topilmadi.</div>
  );
